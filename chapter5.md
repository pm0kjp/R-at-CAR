...
We'll be using a package called "RMySQL".  RMySQL allows you to create a database connection that persists over time and allows you to reach into a MySQL instance, at the server level or at the database level. 
...
We saw a number of databases at UCSC that we can connect to.  Let's choose caePb1, which I chose at random.  This time, connect to the database in the original MySQL connection request.  As a reminder, the host is genome-mysql.cse.ucsc.edu, the user is genome.
